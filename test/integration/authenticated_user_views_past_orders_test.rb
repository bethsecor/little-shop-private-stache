...
class AuthenticatedUserViewsPastOrdersTest < ActionDispatch::IntegrationTest
  test "logged in user can go to orders and see all their past orders" do
    ...
    order_stache_1 = OrderStache.create(order_id: order_1.id, stache_id: stache_1.id, quantity: 2)
    order_stache_2 = OrderStache.create(order_id: order_1.id, stache_id: stache_2.id, quantity: 9)
    order_stache_3 = OrderStache.create(order_id: order_2.id, stache_id: stache_2.id, quantity: 3)
    order_stache_4 = OrderStache.create(order_id: order_2.id, stache_id: stache_3.id, quantity: 4)
    ...
    user.orders.each do |order|
      ...
      order.staches.each do |stache|
        assert page.has_content?("#{stache.name}: #{stache.quantity}")
      end
    end
  end
end
